...
class VASPConfigManager:
    ...
    def get_cluster_config(worker_n):
        ...
        configs = {
            ...
            "brl": {
                "module": "yaml_generator_brl_hfe",
                "resources": {"nodes": 1, "ntasks": 56, "partition": "G1Part_sce",}
            },
        }
...
class VASPWorkflowBuilder:
    ...
    def build_workflow_eletrode(self, input_data: str, entry_data: str, base_incar: Dict[str, Any] = None, parr_incar: Dict[str, Any] = None, **maker_kwargs):
        """
        支持大多数 Maker 
        :param input_data: 结构输入（文件路径或材料ID）
        :param base_incar: 基础 INCAR 参数
        ...
        """
        structure = self._get_structure(input_data)
        working_ion_entry = loadfn(entry_data) 
        #print(structure)
        base_incar = base_incar or VASPConfigManager.get_default_incar()
        parr_incar = parr_incar or {}

        # 调用 Maker 的 make 方法，传递专属参数
        flow = self.maker_instance.make(structure, working_ion_entry=working_ion_entry, **maker_kwargs)
        flow = update_user_incar_settings(flow, base_incar)  # 应用基础 INCAR 参数，所有动态job都可以继承
    ...
    #def _update_job_dec(self, flow):
    #    """修饰 job 用于保存大数据，注意该方式不能处理动态生成的job"""
    #    for job in flow.jobs:
    #        if job.name == self.SPECIAL_JOBS:
    #            incar_updates = {**parr_incar} 
    #            flow = update_user_incar_settings(
    #                flow=flow,
    #                incar_updates=incar_updates,
