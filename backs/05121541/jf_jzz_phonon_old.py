...
class VASPWorkflowBuilder:
    ...
    def build_workflow_phonon(
        ...
        timeout: int = 500,
        max_atom_count: int = 250,
        base_incar: Dict[str, Any] = None,
        parr_incar: Dict[str, Any] = None,
        **maker_kwargs
    ):
        """
        构建声子谱计算的工作流
        :param input_data_s: 结构输入（文件路径或材料ID）
        :param min_length: 超胞最小长度（单位：Å）
        :param max_length: 超胞最大长度（单位：Å）
        :param prefer_90_degrees: 是否偏好90度晶胞
        :param timeout: 超胞计算超时时间（秒）
        :param max_atom_count: 最大允许原子数
        :param base_incar: 基础INCAR参数
        :param parr_incar: 并行计算参数
        :param maker_kwargs: 传递给PhononMaker的专属参数
        """
        try:
            structure = self._get_structure(input_data_s)
        except ValueError as e:
        ...
        # 计算超胞原子数（带超时机制）
        atom_count = self._compute_supercell_atom_count(
            structure,
            min_length=min_length,
            max_length=max_length,
        ...
        if atom_count is None or atom_count > max_atom_count:
            print(f"原子数检查不通过: {'超时' if atom_count is None else f'{atom_count} atoms'}")
            return None
    
        # 配置INCAR参数
        base_incar = base_incar or VASPConfigManager.get_default_incar()
        parr_incar = parr_incar or {}
        born_incar = {**base_incar, "ISMEAR": 0, "LEPSILON": True}
    
        # 创建各阶段计算设置生成器
        kpoints_settings = {"grid_density": 200, "even": True}
        relax_set = TightRelaxSetGenerator(
        ...
        born_set = StaticSetGenerator(
            user_incar_settings=born_incar,
            user_kpoints_settings=kpoints_settings,
            lepsilon=True
        )
    
        # 配置异常处理器
        walltime_handler = WalltimeHandler(
            wall_time=5000,
            buffer_time=500,
        ...
        phonon_maker = PhononMaker(
            min_length=min_length,
            get_supercell_size_kwargs={"max_atoms": max_atom_count},
            bulk_relax_maker=DoubleRelaxMaker.from_relax_maker(
                TightRelaxMaker(input_set_generator=relax_set)
            ),
            static_energy_maker=StaticMaker(input_set_generator=static_set),
            born_maker=DielectricMaker(
                input_set_generator=born_set,
                run_vasp_kwargs={
                    "handlers": [walltime_handler],
                    "max_errors": 5
                }
            ),
            phonon_displacement_maker=PhononDisplacementMaker(
                input_set_generator=static_set
            ),
        ...
        try:
            ...
            print(f"工作流生成失败: {str(e)}")
            return None
    
        # 应用参数设置
        flow = update_user_incar_settings(flow, base_incar)
        # 统一处理 Job 到 Flow 的转换
        if isinstance(flow, Job):
            flow = Flow([flow])
