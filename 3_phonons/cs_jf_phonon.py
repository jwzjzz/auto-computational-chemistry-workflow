...
def run_workflow_from_config():
    ...
    try:
        if maker_config.get('class') == DoubleRelaxMaker.__name__:
            ...
            if phonon_params and 'born_maker' in phonon_params:
                born_maker = phonon_params['born_maker']
                if born_maker is not None:
                    phonon_params['born_maker'] = DielectricMaker() 
            
            custom_maker = PhononMaker(**phonon_params)
            builder = VASPWorkflowBuilder(custom_maker)
            my_flow = builder.build_workflow_phonon(
                input_data=input_data,  # 支持材料ID或文件路径
                base_incar=base_incar,
                parr_incar=parr_incar,
            )
        else:
