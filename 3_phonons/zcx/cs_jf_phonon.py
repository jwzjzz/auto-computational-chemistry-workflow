...
def run_workflow_from_config():
    ...
    phonon_params = maker_config.get("phonon_maker")
    #
    ...
    insert_maker_kwargs = maker_kwargs.get('insert_maker_kwargs')
    #
    try:
        if maker_config.get('class') == DoubleRelaxMaker.__name__:
            ...
            my_flow = builder.build_workflow(
                ...
                base_incar=base_incar,
                parr_incar=parr_incar,
                **insert_maker_kwargs  # 动态传递 Maker 专属参数（如插层参数）
            )
        ...
        elif maker_config.get('class') == PhononMaker.__name__:
            custom_walltime_handler = WalltimeHandler(
            wall_time=50000,
            buffer_time=500,
            electronic_step_stop=True
            )
            custom_walltime_handler.is_terminating = True
            custom_maker = PhononMaker(
            ...
            born_maker=DielectricMaker(
            	...
            	"wall_time": None, 
            	"max_errors": 5,
            	"handlers":[custom_walltime_handler]}),
            use_symmetrized_structure="primitive",
            )
            builder = VASPWorkflowBuilder(custom_maker)
            my_flow = builder.build_workflow_phonon(
                input_data_s=input_data,  # 支持材料ID或文件路径
                base_incar=base_incar,
                parr_incar=parr_incar,
            )
            #
